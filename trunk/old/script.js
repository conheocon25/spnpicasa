...
//resize
var resize = getCookie('resize') ? getCookie('resize') : 6;
...
function getResize() {
    return getCookie('resize') ? getCookie('resize') : 6; 
}
...
function transferlocal(id) {
    transfer_id = (!id) ? transfer_id : id;
    var _wtm = document.getElementById('watermark').checked ? '1' : '0';
    text = document.getElementById('fileuploadpicasa').value;
	alert (text);
    var re = /.*?(\[IMG\])?(https?[^\[\'\"\;]*\.(jpg|png|bmp|gif|jpeg))(\[\/IMG\])?/ig;
    var m = re.exec(text);
    var html = "";
